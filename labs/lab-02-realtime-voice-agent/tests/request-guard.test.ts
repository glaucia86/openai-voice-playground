...
describe("guardApiRequest", () => {
  it("accepts a same-origin request and returns a quota", () => {
    ...
    expect(guardApiRequest(request, "realtime-token")).toMatchObject({ allowed: true, remaining: 9 });
  });
  ...
  it("blocks explicit cross-site browser requests", () => {
    ...
    expect(() => guardApiRequest(request, "realtime-token")).toThrowError(AppError);
    try {
      guardApiRequest(request, "realtime-token");
    } catch (error) {
  ...
  it("requires the optional deployment access token", () => {
    ...
    expect(() => guardApiRequest(unauthorized, "realtime-token")).toThrow();

    const authorized = new Request("https://voice.example.com/api/realtime/token", {
      headers: { authorization: "Bearer a-long-shared-token" },
    });
    expect(guardApiRequest(authorized, "realtime-token").allowed).toBe(true);
  });
  ...
  it("honors a configured canonical origin", () => {
    ...
    expect(() => guardApiRequest(request, "realtime-token")).toThrow();
  });
  ...
  it("rejects an invalid origin header", () => {
    ...
    expect(() => guardApiRequest(request, "realtime-token")).toThrowError(
  ...
  it("enforces the local quota", () => {
    for (let index = 0; index < 10; index += 1) {
      guardApiRequest(new Request("https://voice.example.com/api/realtime/token"), "realtime-token");
    }
